...
public class Joint
{
    ...
    public void BuildHierarchy(GameObject baseObject)
    {
        ...
        if (this.parent != null)
        {
            ...
            if (this.parent.jointObj.transform.GetChild(0).localScale.x <= 1.01)
            {
                this.parent.jointObj.transform.GetChild(0).localScale *= this.offset.magnitude;
            }
        }
        this.jointObj.transform.localScale = new Vector3(1, 1, 1);
        this.jointObj.transform.GetChild(0).localScale = 1 * (new Vector3(1, 1, 1));
        //this.jointObj.transform.GetChild(0).localScale *= this.parent.offset.magnitude;
        ...
        if (this.children.Count > 0)
        {
            ...
            rotation = Quaternion.FromToRotation(this.jointObj.transform.GetChild(0).forward, upDir);
            this.jointObj.transform.GetChild(0).rotation = rotation * this.jointObj.transform.GetChild(0).rotation;
        }
        else
        {
            Vector3 endPoint = new Vector3(-endNote.x, endNote.y, endNote.z) / 100;
            rotation = Quaternion.FromToRotation(this.jointObj.transform.GetChild(0).forward, endPoint);
            jointInfo.endPoints.Add(Vector3.forward * endPoint.magnitude);
            this.jointObj.transform.GetChild(0).localScale *= endPoint.magnitude * 100;
            this.jointObj.transform.GetChild(0).rotation = rotation * this.jointObj.transform.GetChild(0).rotation;
        }
    ...
    public List<string> ZeroFrame()
    {
        ...
        foreach (Joint child in this.children)
        {
            var czeros = child.ZeroFrame();
            zeros.AddRange(czeros);
        }
        return zeros;
    }
    ...
    {
        float height = this.GetJointPosition().y;
        foreach (Joint c in this.children)
        {
            float cHeight = c.GetLowestHeight();
            if(cHeight <= height)
            {
                height = cHeight;
            }
...
public class ApplyBVH : MonoBehaviour
{
    ...
    private Joint root;
    private int motionFrame = 0;
    private int motionsStart = 0;
    public int fps = 30;
    public float lastRender = 0;
    public float renderCounter = 0;
    ...
    private void LateUpdate()
    {
        if (start && isInit)
        {
            if ( (renderCounter + Time.deltaTime) > (lastRender + 1.0f / fps))
            {
                lastRender = renderCounter = renderCounter + Time.deltaTime;
                // select next frame. 
                if (motionFrame >= this.lines.Count)
                {
                    //this.Init(file);
                    motionFrame = motionsStart;
                }
            ...
            {
                renderCounter = renderCounter + Time.deltaTime;
            }
        }
