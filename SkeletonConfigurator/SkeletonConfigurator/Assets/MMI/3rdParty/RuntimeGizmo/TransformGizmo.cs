...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		public TransformSpace GetProperTransformSpace()
		{
            ...
            return transformType == TransformType.Scale ? TransformSpace.Global : space;
        }
					...
					else if(transType == TransformType.Rotate)
					{
						...
						for(int i = 0; i < targetRootsOrdered.Count; i++)
						{
							...
							if(pivot == TransformPivot.Pivot)
							{
								...
								JointController jointController = target.GetComponent<JointController>();
								if(jointController != null)
									jointController.Rotate(rotationAxis, rotateAmount, Space.World);
							}
							else if(pivot == TransformPivot.Center)
							{
		...
		{
			if (Input.GetKeyDown(KeyCode.F) && mainTargetRoot != null && flyCam != null)
			{
				flyCam.target = mainTargetRoot;
				flyCam.distance = 1;
			}
		}
	}
